...
const HREmpUpdatingForm = ({ employee, open, handleClose, onSave }) => {
    ...
    const { empId } = useParams(); // Use useParams to get empId
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            const response = await axios.put(
                `${APIbackendURl.basePointUrl}applicationservice/Admin/PortalMaster/UserMaster/Edit`,
                { empId, ...values }, // Include empId in the request body
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
            ...
            if (response.status === 200) {
                ...
                handleOpenAlert('Details updated successfully successfully', 'success');

                onSave("Details updated successfully", "success"); // Call the onSave function with success message
                setIsSaving(true);
                ...
                // onSave("Error updating details", "error"); // Call the onSave function with error message
            }
        } catch (error) {
            console.error('Error updating details', error);
            // onSave("Error updating details", "error"); // Call the onSave function with error message
            handleOpenAlert('Error updating details', 'error');
    ...
    const formik = useFormik({
        initialValues: {
            empId: empId, // Use the empId obtained from useParams
            approverId: "",
            approverName: employee.approverName,
            authorizerEmpId: "",
            authorizerEmpName: employee.authorizerEmpName,
            base64: "",
            ...
            documentExt: "",
            documentType: "photo",
            emailOff: employee.emailOff,
            emailOthers: employee.emailOthers,
            empCatName: "",
            // empId: 0,
            empcatId: "",
            // endDate: "",
            ...
            jobSpecification: employee.jobSpecification,
...
export default HREmpUpdatingForm; 
